...
/**
 * find_square_local.js (fixed)
 *
 * Robust helper to find/search an address on map.coveragemap.com
 * - Removed unsafe navigator.permissions override causing "Illegal invocation"
 * - Kept network / console logging, artifact saving, robust selectors and retries
 */
...
async function findSquare(arg1, arg2) {
  ...
  // Safe stealth-like settings (no risky permissions patch)
  try {
    ...
    console.warn('setUserAgent failed', e && e.message ? e.message : e);
  }

  // Use evaluateOnNewDocument for minimal safe tweaks only
  try {
    if (page.evaluateOnNewDocument) {
      await page.evaluateOnNewDocument(() => {
        try { Object.defineProperty(navigator, 'webdriver', { get: () => false }); } catch(e){}
        try { window.chrome = window.chrome || { runtime: {} }; } catch(e){}
        // DO NOT override navigator.permissions.query — it can cause Illegal invocation in some environments.
      });
    }
  } catch (e) {
    console.warn('evaluateOnNewDocument failed (ignored):', e && e.message ? e.message : e);
  }
  ...
  // Also build a HAR-like requests array for artifact saving
  const requests = [];
  const pushReq = (r) => { try { requests.push(r); if (requests.length > 2000) requests.shift(); } catch(e){} };
  ...
  if (page.on) {
    ...
    page.on('response', resp => {
      try {
        pushReq({ type: 'response', status: resp.status(), url: resp.url(), ts: Date.now() });
      } catch (e) {}
      onResponse(resp);
    });
    page.on('requestfinished', async req => {
      try {
        ...
        pushReq({ type: 'requestfinished', url: req.url(), status: resp ? resp.status() : null, ts: Date.now() });
        // log concise
  ...
  try {
    ...
    // attempt to save partial artifacts
    try {
  ...
  if (!inputHandle) {
    try {
      ...
      const htmlPath = `${debugPrefix}.html`;
      const shotPath = `${debugPrefix}.png`;
      const reqPath = `${debugPrefix}.requests.json`;
      try { fs.writeFileSync(htmlPath, html); } catch(e){}
      try { await (page.screenshot ? page.screenshot({ path: shotPath, fullPage: true }) : Promise.resolve()); } catch(e){}
      try { fs.writeFileSync(reqPath, JSON.stringify(requests.slice(-2000), null, 2)); } catch(e){}
      console.error('Search input not found on page. Saved debug artifacts:', htmlPath, shotPath, reqPath);
